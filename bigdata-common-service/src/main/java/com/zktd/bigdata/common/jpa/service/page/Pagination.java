...
public class Pagination<T> extends PageInfo<T> {
	...
	private static final long serialVersionUID = 5226857004030257971L;
						...
						if (field.getType() == Integer.class) {
							...
							if (!"".equals(page.getSearch().getValue())) {
								...
								try {
									...
									ReflectionUtils.setField(field, t, Long.parseLong(value));
								} catch (NumberFormatException e) {
								}
							}
						} else if (field.getType() == Date.class) {
...
/*		for (int i = 0; null != page.getOrder() && i < page.getOrder().size(); i++) {
	...
	public static <T> Sort getOrderBydataTable(Pagination<T> page) {
		...
		for (int i = 0; null != page.getOrder() && i < page.getOrder().size(); i++) {
			...
			if("asc".equals(order.getDir())){
				orderlist.add(new Sort.Order(Direction.ASC, columns.getData()));
			}else{
				orderlist.add(new Sort.Order(Direction.DESC, columns.getData()));
			}
	...
	public static <T> Sort.Order getStringOrderBydataTable(Pagination<T> page) {
		...
		for (int i = 0; null != page.getOrder() && i < page.getOrder().size(); i++) {
			...
			Columns columns = page.getColumns().get(order.getColumn());
			if("asc".equals(order.getDir())){
				sortorder=new Sort.Order(Direction.ASC, columns.getData());
				break;
			}else{
				sortorder=new Sort.Order(Direction.DESC, columns.getData());
				break;
			}
